...
public class DerelictTowerGrassPieces {
	...
	private static final ImmutableMap<Identifier, BlockPos> CENTER_TOP_OFFSETS = ImmutableMap.of(DERELICT_TOWER_TOP, new BlockPos(6, 5, 6), DERELICT_TOWER_BOTTOM, new BlockPos(3, 31, 3));
	private static final ImmutableMap<Identifier, BlockPos> CORNER_RELATIVE_POSITIONS = ImmutableMap.of(DERELICT_TOWER_TOP, new BlockPos(4, 0, 4), DERELICT_TOWER_BOTTOM, BlockPos.ORIGIN);
	...
	public static class Piece extends SimpleStructurePiece {
		...
		public Piece (StructureManager structureManager, CompoundTag tag) {
			super(RegistryHandler.DERELICT_TOWER_GRASS_PIECE, tag);
		...
		@Override
		public boolean generate (IWorld world, Random random, BlockBox boundingBox, ChunkPos chunkPos) {
			StructurePlacementData placementData = (new StructurePlacementData().setRotation(BlockRotation.NONE).setMirrored(BlockMirror.NONE).setPosition(CENTER_TOP_OFFSETS.get(this.structurePart)));
			...
			if (this.structurePart.equals(DERELICT_TOWER_BOTTOM)) {
				...
				for (int i = 0; i < 5; ++i) {
					for (int j = 0; j < 5; ++j) {
						height = world.getTop(Heightmap.Type.WORLD_SURFACE_WG, blockpos1.getX() + 2 + i, blockpos1.getZ() + 2 + j);
						if (height < minHeight) {
							minHeight = height;
						}
					}
			...
			return super.generate(world, random, boundingBox, chunkPos);
		}
	}
}